    ...
    public static void main(String[] args) {
        ...
        if (employeesWith7ConsecutiveDays.size() != 0) {
            for (Employee_ employee : employeesWith7ConsecutiveDays) {
                System.out
                        .println("Name: " + employee.getEmployeeName() + ", Position ID: " + employee.getPositionID());
            }
        } else {
            System.out.println();
        ...
        List<Employee_> employeesWithLessThan10HoursBetweenShifts = getEmployeesWithLessThan10HoursBetweenShifts(
                employees);
        System.out.println(
                "---------------------------------------------------------------------------------------------");
        ...
        System.out.println();
        System.out.println(
                "Total Employees who have less than 10 hours between shifts but greater than 1 hour are : " + cnt);
        System.out.println(
                "---------------------------------------------------------------------------------------------");

        List<Employee_> employeesWithMoreThan14HoursInSingleShift = getEmployeesWithMoreThan14HoursInSingleShift(
                employees);
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (data.length >= 8) {
                    ...
                    if (parseDate(data[2])[0].equals("Time") || parseDate(data[2])[0].equals("")
                            || parseDate(data[3])[0].equals(""))
                        continue;
                    ;
                    ...
                    Employee_ employee = new Employee_(positionID, timeInDay, timeOutDay, employeeName, fileNumber,
                            timeHours);
    ...
    private static List<Employee_> getEmployeesWithLessThan10HoursBetweenShifts(List<Employee_> employees) {
        ...
        for (List<Employee_> employeeList : employeesByFileNumber.values()) {
            for (int i = 0; i < employeeList.size() - 1; i++) {
                ...
                try {
                    hoursBetweenShifts = Integer.parseInt(nextEmployee.getTimeInDay())
                            - Integer.parseInt(currentEmployee.getTimeOutDay());
                } catch (Exception e) {
                    continue;
                }
